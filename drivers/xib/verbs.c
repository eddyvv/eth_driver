 ...
 *
 */
int xib_alloc_qp_buffers(struct ib_device *ibdev, struct xib_qp *qp)
{
	 ...
	 * allocate RQ buffer
	 */
	rq->rq_ba_v = dma_alloc_coherent(&xib->pdev->dev,
	...
	dev_dbg(&xib->ib_dev.dev, "%s: rq_ba_v: %px rq_ba_p : %llx qpn: %d", __func__, rq->rq_ba_v,
	...
	qp->sq_ba_v = dma_alloc_coherent(&xib->pdev->dev,
	...
	dev_dbg(&xib->ib_dev.dev, "%s: sq_ba_v: %px sq_ba_p : %llx qpn: %d", __func__, qp->sq_ba_v,
