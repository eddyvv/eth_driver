...
#define DRIVER_NAME		"xilinx_axienet"
             ...
             const struct pci_device_id *id)
{
    ...
    dev->regs_start = pci_resource_start(pdev, 0);
    xt_printk("dev->regs_start = 0x%x\n",*(unsigned int *)&dev->regs_start);
    if (!dev->regs_start){
...
static void xtenet_pci_remove(struct pci_dev *pdev)
{

}

static struct pci_driver xtenet_pci_driver = {
    .name     = DRIVER_NAME,
    .id_table   = xtenet_pci_tbl,
    .probe      = xtenet_pci_probe,
    .remove     = xtenet_pci_remove,
};
#endif /* CONFIG_PCI */
...
static int __init xtenet_init_module(void)
{
    ...
    xt_printk("%s start\n\n",__func__);
#ifdef CONFIG_PCI
    xt_printk("pci_register_driver start\n\n");
	rc = pci_register_driver(&xtenet_pci_driver);
	if (rc)
		return rc;
#endif
	rc = platform_driver_register(&axienet_driver);
	if (rc)
		goto err;
    xt_printk("%s end\n",__func__);
	return 0;
...
err:
#ifdef CONFIG_PCI
	pci_unregister_driver(&xtenet_pci_driver);
#endif
	return rc;
}
